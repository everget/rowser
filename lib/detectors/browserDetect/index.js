...
module.exports = function* browserDetect(ua, summary) {
  yield /edg([ea]|ios)/i.test(ua) && { // chrome.+?
    msedge: t,
    name: 'Microsoft Edge',
    version: getSecondMatch(new RegExp(`edg([ea]|ios)\/${DOT_NUMBERS_2}`, 'i'), ua),
  }
  ...
  yield (summary.tizen || /tizen\s?browser/i.test(ua)) && {
    name: 'Tizen Browser',
    tizenbrowser: t,
    version: getFirstMatch(reVersion, ua) || getSecondMatch(new RegExp(`(?:(tizen|slp)\\s?browser)[ \/]${DOT_NUMBERS_2}`, 'i'), ua),
  }
  ...
  yield /msie|trident/i.test(ua) && {
    ...
    version: /iemobile/i.test(ua) ? getFirstMatch(new RegExp(`(?:iemobile)[ \/]${DOT_NUMBERS_2}`, 'i'), ua) : getFirstMatch(new RegExp(`(?:msie |rv:)${DOT_NUMBERS_2}`, 'i'), ua),
  }
  ...
  yield /omniweb/i.test(ua) && {
    omniweb: t,
    name: 'OmniWeb',
    version: getFirstMatch(new RegExp(`omniweb\/v${DOT_NUMBERS_2}`, 'i'), ua),
  }
