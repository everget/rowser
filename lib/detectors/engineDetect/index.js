...
module.exports = function* engineDetect(ua, summary, canUseDOM) {
  ...
  yield !summary.opera && /msie|trident/.test(ua) && {
    engine: 'Trident',
    engineversion: getMatchByIndex(1, new RegExp(`trident/${DOT_NUMBERS_2}`), ua),
    trident: t,
  };
  ...
  yield (/fxios/.test(ua) || /(apple)?webkit/.test(ua)) && {
    engine: 'WebKit',
    engineversion: getMatchByIndex(2, new RegExp(`(apple)?webkit/${DOT_NUMBERS_2}`), ua),
    webkit: t,
  };

  yield /goanna/.test(ua) && {
    engine: 'Goanna',
    engineversion: getMatchByIndex(1, new RegExp(`goanna/${DOT_NUMBERS_1}`), ua),
    goanna: t,
  }

  yield !/opr\/|opios/.test(ua) && /gecko(\/)?/.test(ua) && {
    engine: 'Gecko',
    engineversion: getMatchByIndex(1, new RegExp(`rv:${DOT_NUMBERS_1}`), ua),
    gecko: t,
  };

  yield summary.opera && {
    engine: 'Presto',
    engineversion: getMatchByIndex(1, new RegExp(`presto/${DOT_NUMBERS_1}`), ua),
    presto: t,
  };
}
